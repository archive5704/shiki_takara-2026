...
function renderColorPalette(){
  ...
  COLORS.forEach(c=>{
    ...
    btn.innerHTML='<span class="color-chip-swatch" style="background:'+c.color+'"></span><span>'+esc(c.label)+'</span>';
    btn.onclick=()=>{
...
function updateColorPaletteSelection(){
  if(!els.colorPalette)return;
  els.colorPalette.querySelectorAll(".color-chip").forEach(btn=>{
    btn.classList.toggle("active",btn.dataset.value===form.colorSelect.value);
  });
}
...
els.addBtn.onclick=()=>{startNew();if(els.editor.classList.contains("collapsed"))toggleEditor(true);};
