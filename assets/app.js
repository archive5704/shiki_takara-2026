...
async function reloadAll(){
  ...
  try{
    const fetchPromise=Promise.all([ShikiSupabase.fetchCategories(),ShikiSupabase.fetchTreasures()]);
    const timeoutPromise=new Promise((_,reject)=>setTimeout(()=>reject(new Error("Supabase読込がタイムアウトしました。接続・RLS・grantをご確認ください。")),20000));
    const [cats,trs]=await Promise.race([fetchPromise,timeoutPromise]);
    categories=cats.map(c=>({id:c.id,name:c.name||"",sort_order:Number(c.sort_order||0)}));
    items=trs.map(t=>({id:t.id,name:t.name||"",start:t.start_date||"",end:t.end_date||"",category_id:t.category_id||"",color:t.color||"rose",detail:t.detail||"",image_url:t.image_url||""}));
    renderPickers();
    renderRoadmap(monthItems());
