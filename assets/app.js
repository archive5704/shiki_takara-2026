...
function renderSelects(){els.filterSelect.innerHTML="";const all=document.createElement("option");all.value="all";all.textContent="すべて";all.selected=activeCategory==="all";els.filterSelect.appendChild(all);categories.forEach(c=>{const o=document.createElement("option");o.value=c.id;o.textContent=c.name;o.selected=activeCategory===c.id;els.filterSelect.appendChild(o);});form.categorySelect.innerHTML="";categories.forEach(c=>{const o=document.createElement("option");o.value=c.id;o.textContent=c.name;form.categorySelect.appendChild(o);});form.colorSelect.innerHTML="";COLORS.forEach(c=>{const o=document.createElement("option");o.value=c.value;o.textContent=c.label;form.colorSelect.appendChild(o);});}
...
function startNew(){editingId=null;els.editorTitle.textContent="新しく記録する";els.saveBtn.textContent="保存する";clearMsg();form.name.value="";form.start.value="";form.end.value="";form.categorySelect.value=categories[0]?.id||"";form.colorSelect.value=COLORS[0].value;form.detail.value="";resetImage();}
function fillForm(item){editingId=item.id;els.editorTitle.textContent="記録を修正する";els.saveBtn.textContent="修正を保存";clearMsg();form.name.value=item.name||"";form.start.value=item.start||"";form.end.value=item.end||"";form.categorySelect.value=item.category_id||"";form.colorSelect.value=item.color||COLORS[0].value;form.detail.value=item.detail||"";currentImageUrl=item.image_url||"";selectedImageFile=null;removeImageFlag=false;if(els.imageFile)els.imageFile.value="";setPreview(currentImageUrl);setImageStatus(currentImageUrl?"現在の画像を使用中":"画像なし");if(els.editor.classList.contains("collapsed"))toggleEditor(true);}
...
els.prevBtn.onclick=()=>{if(currentMonth===1){currentMonth=12;currentYear--}else currentMonth--;renderYears();renderRoadmap();};
els.nextBtn.onclick=()=>{if(currentMonth===12){currentMonth=1;currentYear++}else currentMonth++;renderYears();renderRoadmap();};
...
(async function boot(){await AppAuth.init();applyAdminState();renderSelects();startNew();await reloadAll();})();
